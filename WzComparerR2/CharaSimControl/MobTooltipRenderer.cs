    ...
    public class MobTooltipRenderer : TooltipRender
    {
        ...
        public override Bitmap Render()
        {
            ...
            if (MobInfo.IsQuestCountGroupMob)
            {
                ...
                {
                    sbExt.Append($"[任务怪物设置: 角色等级±20级怪物] ");
                }
                else if (MobInfo.ChangeableMob)
                {
                    sbExt.Append($"[任务怪物设置: 精英怪] ");
                }
                else if (MobInfo.Filters != 0)
                {
                    switch (MobInfo.Filters)
                    {
                        case 1: sbExt.Append($"[任务怪物设置: 星之力地区怪物] "); break;
                        case 2: sbExt.Append($"[任务怪物设置: 神秘河地区怪物] "); break;
                        case 5: sbExt.Append($"[任务怪物设置: 星之力地区精英怪] "); break;
                        case -10: sbExt.Append($"[任务怪物设置: 神秘/原初之力地区怪物] "); break;
                        default: sbExt.Append($"[任务怪物设置: 特殊] "); break;
                    }
                }
                else
                {
                    sbExt.Append($"[任务怪物设置: {MobInfo.QuestCountGroupMobID.Count}体] ");
                }
                if (sbExt.Length > 1)
                {
                ...
                {
                    propBlocks.Add(PrepareText(g, "按[-]/[+]进行切换。", GearGraphics.ItemDetailFont, Brushes.White, 0, picY));
                }
