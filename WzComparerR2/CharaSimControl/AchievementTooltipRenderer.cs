...
namespace WzComparerR2.CharaSimControl
{
    public class AchievementTooltipRenderer : TooltipRender
    {
        ...
        public override Bitmap Render()
        {
            ...
            var timeConvertFormat = "yyyy年 MM月 dd日 HH时 mm分";
                ...
                && DateTime.TryParseExact(this.Achievement.End, timeParseFormat, CultureInfo.InvariantCulture, DateTimeStyles.None, out var end))
            {
                ...
                GearGraphics.DrawString(g, time, GearGraphics.EquipDetailFont, null, 84, 550, ref picHD, 15, defaultColor: ((SolidBrush)GearGraphics.AchievementPeriodBrush).Color);
            }
            var desc = sr.Desc;
            if (!string.IsNullOrEmpty(desc))
            {
                GearGraphics.DrawString(g, desc, Translator.IsKoreanStringPresent(desc) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.EquipDetailFont, null, 84, 550, ref picHD, 15, defaultColor: GearGraphics.GrayColor2);
            }
            ...
            if (this.Achievement.ShowMissions)
            {
                ...
                foreach (var mission in this.Achievement.Missions)
                {
                    ...
                    TextRenderer.DrawText(g, Compact(g, mission, 235), Translator.IsKoreanStringPresent(mission) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.EquipDetailFont, new Point(32 + dx * col, picH + 1), Color.White, TextFormatFlags.NoPadding);
            ...
            if (!string.IsNullOrEmpty(category))
            {
                ...
                GearGraphics.DrawString(g, $"分类: {category}", GearGraphics.EquipDetailFont, null, 32, 550, ref picH, bg_pattern.Height, defaultColor: GearGraphics.GrayColor2);
            }
            ...
            if (this.Achievement.PriorIDs.Count > 0)
            {
                ...
                }))}", 520), Translator.IsKoreanStringPresent(sr.Name) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.EquipDetailFont, null, 32, 550, ref picH, bg_pattern.Height, defaultColor: GearGraphics.GrayColor2);
            }
            ...
            {
                g.DrawImage(checkIncomplete, 16, picH);
                GearGraphics.DrawString(g, "不在成就界面显示", GearGraphics.EquipDetailFont, null, 32, 550, ref picH, bg_pattern.Height, defaultColor: GearGraphics.GrayColor2);
            }
            ...
            if (this.Achievement.HasRewards)
            {
                ...
                TextRenderer.DrawText(g, Compact(g, reward.Desc, 448), Translator.IsKoreanStringPresent(reward.Desc) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.EquipDetailFont, new Point(104, picH + 28), ((SolidBrush)GearGraphics.AchievementRewardBrush).Color, TextFormatFlags.NoPadding);
            }
        ...
        {
            Size s = TextRenderer.MeasureText(g, text, GearGraphics.EquipDetailFont, new Size(int.MaxValue, int.MaxValue), TextFormatFlags.NoPadding);
            ...
            while (seg > 1)
            {
                ...
                s = TextRenderer.MeasureText(g, tst, GearGraphics.EquipDetailFont, new Size(int.MaxValue, int.MaxValue), TextFormatFlags.NoPadding);
