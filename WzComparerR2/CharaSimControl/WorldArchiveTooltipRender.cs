    ...
    public class WorldArchiveTooltipRender : TooltipRender
    {
        ...
        public override Bitmap Render()
        {
            ...
            using (Graphics g = Graphics.FromImage(bmp1))
            {
                ...
                {
                    GearGraphics.DrawPlainText(g, "[警告]以下信息已过时，不能反映当前版本的状态。", GearGraphics.ItemDetailFont, Color.White, 13, 272, ref height, 16);
                    height += 4;
                ...
                {
                    GearGraphics.DrawPlainText(g, "来自NPC对话:", GearGraphics.ItemDetailFont, Color.White, 13, 272, ref height, 16);
                    height += 4;
            ...
            using (Graphics g = Graphics.FromImage(bmp2))
            {
                ...
                int picH = 12;
                // GearGraphics.DrawPlainText(g, "世界档案", GearGraphics.ItemDetailFont, Color.FromArgb(255, 255, 255), 8, 130, ref picH, 13);
                ...
                if (!string.IsNullOrEmpty(MonsterBookMessage))
                {
                    GearGraphics.DrawPlainText(g, "[警告]以下信息已过时，不能反映当前版本的状态。", GearGraphics.ItemDetailFont, Color.Orange, 13, 272, ref picH, 16);
                    picH += 4;
                    foreach (var i in SplitLine(MonsterBookMessage))
                    {
                        GearGraphics.DrawPlainText(g, i, Translator.IsKoreanStringPresent(i) ? GearGraphics.KMSItemDetailFont : GearGraphics.ItemDetailFont, Color.White, 13, 272, ref picH, 16);
                ...
                if (!string.IsNullOrEmpty(NpcQuoteMessage))
                {
                    GearGraphics.DrawPlainText(g, "NPC对话", GearGraphics.ItemDetailFont, Color.FromArgb(204, 255, 0), 13, 272, ref picH, 16);
                    picH += 4;
                    foreach (var i in SplitLine(NpcQuoteMessage))
                    {
                        switch (i.Trim())
