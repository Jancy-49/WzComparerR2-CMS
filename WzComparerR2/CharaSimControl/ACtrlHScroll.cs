...
{
    public class ACtrlHScroll : AControl
    {
        public ACtrlHScroll()
        {
        ...
        public override void Draw(Graphics g)
        {
            ...
            if (picBase != null)
            {
                ...
                for (int w = 0; w < Size.Width; w += curBmp.Bitmap.Size.Width)
                {
                    g.DrawImage(curBmp.Bitmap, Location.X + w, Location.Y);
                }
                g.ResetClip();
            }
            if (btnPrev != null)
            ...
            if (btnThumb != null)
            {
                btnThumb.Location = new Point(calcThumbLocationX(), 0);
                btnThumb.Draw(g, this.Location);
            }
        }

        private int calcThumbLocationX()
        {
            if (this.minimum == this.maximum)
                return 0;
            int totalWidth = this.Size.Width - this.btnPrev.Size.Width - this.btnNext.Size.Width - this.btnThumb.Size.Width;
            int thumbX = totalWidth * this.value / (this.maximum - this.minimum);
            return thumbX + this.btnPrev.Size.Width;
        }
        ...
        private void scrolling(Point mouseLocation)
        {
            if (this.minimum == this.maximum)
                return;
            Point origin = new Point(this.Location.X + this.btnPrev.Size.Width + this.btnThumb.Size.Width / 2, this.Location.Y);
            Size size = new Size(this.Size.Width - this.btnPrev.Size.Width - this.btnNext.Size.Width - this.btnThumb.Size.Width, this.Size.Height);
            Rectangle scrollingRect = new Rectangle(origin, size);
            this.Value = (int)Math.Round(1.0 * (this.maximum - this.minimum) * (mouseLocation.X - scrollingRect.X) / scrollingRect.Width);
        }
