    ...
    public class QuestTooltipRenderer : TooltipRender
    {
        ...
        private Bitmap RenderBase()
        {
            ...
            if (!string.IsNullOrEmpty(this.Quest.Name))
            {
                if (Translator.IsTranslateEnabled)
                {
                    ...
                    TextRenderer.DrawText(g, translatedQuestName, Translator.IsKoreanStringPresent(translatedQuestName) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.ItemDetailFont, new Point(left, 72), Color.White, TextFormatFlags.NoPadding);
                }
                else
                {
                    TextRenderer.DrawText(g, Compact(g, this.Quest.Name, 200), Translator.IsKoreanStringPresent(this.Quest.Name) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.ItemDetailFont, new Point(left, 72), Color.White, TextFormatFlags.NoPadding);
                }
            }
            // 권장 레벨
            if (this.Quest.Lvmin > 0)
            {
                ...
                {
                    TextRenderer.DrawText(g, $"推荐等级: {this.Quest.Lvmin}以上", GearGraphics.ItemDetailFont, new Point(left, 90), ((SolidBrush)GearGraphics.QuestBrushDefault).Color, TextFormatFlags.NoPadding);
                }
                else
                {
                    var lvMax = this.Quest.Lvmax > 0 ? this.Quest.Lvmax : this.Quest.Lvmin + 4;
                    TextRenderer.DrawText(g, $"推荐等级: {this.Quest.Lvmin}～{lvMax}", GearGraphics.ItemDetailFont, new Point(left, 90), ((SolidBrush)GearGraphics.QuestBrushDefault).Color, TextFormatFlags.NoPadding);
                }
            }
            if (npcImage.Bitmap != null)
            {
            ...
            if (state == 1)
            {
                ...
                if (this.Quest.Check1NpcID > 0) target1.Add($@"{targetNpcPrefix}#$p{GetNpcName(this.Quest.Check1NpcID)}#");

                if (!string.IsNullOrEmpty(this.Quest.PlaceSummary)) target2.Add($@"进行地图 :\n{this.Quest.PlaceSummary}");
                ...
                if (targetr.Count > 0)
                {
                    ...
                    GearGraphics.DrawString(g, replaced, Translator.IsKoreanStringPresent(replaced) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.ItemDetailFont, questColorTable, questFontTable, this.ImageTable, 29, 293, ref picH, 18, alignment: Text.TextAlignment.Left, defaultColor: ((SolidBrush)GearGraphics.QuestBrushDefault).Color);
                    ClearImageTable();
            ...
            if (!string.IsNullOrEmpty(desc))
            {
                ...
                switch (Translator.DefaultPreferredLayout)
                {
                    case 1:
                        ...
                        GearGraphics.DrawString(g, tReplaced, Translator.IsKoreanStringPresent(tReplaced) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.ItemDetailFont, questColorTable, questFontTable, this.ImageTable, 29, 293, ref picH, 18, alignment: Text.TextAlignment.Left, defaultColor: ((SolidBrush)GearGraphics.QuestBrushDefault).Color);
                        GearGraphics.DrawString(g, replaced, Translator.IsKoreanStringPresent(replaced) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.ItemDetailFont, questColorTable, questFontTable, this.ImageTable, 29, 293, ref picH, 18, alignment: Text.TextAlignment.Left, defaultColor: ((SolidBrush)GearGraphics.QuestBrushDefault).Color);
                        break;
                    case 2:
                        tReplaced = Translator.TranslateString(replaced) + "\r\n\r\n";
        ...
        private void DrawRewardItems(Graphics g, QuestReward r, int h)
        {
            ...
            if (r.Exp > 0)
            {
                ...
                {
                    int expValueLength = TextRenderer.MeasureText($"{r.Exp.ToString("N0")}", GearGraphics.ItemDetailFont).Width;
                    TextRenderer.DrawText(g, $"{r.Exp.ToString("N0")}", GearGraphics.ItemDetailFont, new Point(x, y - 4), ((SolidBrush)GearGraphics.WhiteBrush).Color, TextFormatFlags.NoPadding);
            ...
            if (r.Meso > 0)
            {
                ...
                {
                    int mesoValueLength = TextRenderer.MeasureText($"{r.Meso.ToString("N0")}", GearGraphics.ItemDetailFont).Width;
                    TextRenderer.DrawText(g, $"{r.Meso.ToString("N0")}", GearGraphics.ItemDetailFont, new Point(x, y - 4), ((SolidBrush)GearGraphics.WhiteBrush).Color, TextFormatFlags.NoPadding);
        ...
        private string ConvertDateWZ2(string info)
        {
            ...
            {
                timeConvertFormat = "{YYYY}年 {MM}月 {DD}日 {hh}时 {mm}分";
            }
        ...
        {
            Size s = TextRenderer.MeasureText(g, text, Translator.IsKoreanStringPresent(text) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.ItemDetailFont, new Size(int.MaxValue, int.MaxValue), TextFormatFlags.NoPadding);
            ...
            while (seg > 1)
            {
                ...
                s = TextRenderer.MeasureText(g, tst, Translator.IsKoreanStringPresent(text) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.ItemDetailFont, new Size(int.MaxValue, int.MaxValue), TextFormatFlags.NoPadding);
