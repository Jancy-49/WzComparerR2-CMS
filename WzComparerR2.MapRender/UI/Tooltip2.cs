...
namespace WzComparerR2.MapRender.UI
{
    class Tooltip2
    {
        ...
        private TooltipContent DrawItem(GameTime gameTime, RenderEnv env, PortalItem item)
        {
            ...
            sb.Append("名称: ").AppendLine(item.PName);
            ...
            sb.Append("移动地图: ").Append(item.ToMap);
            if (item.ToMap != null)
            {
            ...
            sb.Append("移动传送门: ").AppendLine(item.ToName);

            if (!string.IsNullOrEmpty(item.Script))
            {
                sb.Append("文本: ").AppendLine(item.Script);
        ...
        private TooltipContent DrawItem(GameTime gameTime, RenderEnv env, ReactorItem item)
        {
            ...
            sb.Append("名称: ").AppendLine(item.ReactorName);
            sb.Append("时间: ").Append(item.ReactorTime).AppendLine();

            sb.Append("状态: ").Append(item.View.Stage);
