                ...
                case RenderObjectType.Npc:
                    {
                        ...
                        foreach (var kv in p.Actions)
                        {
                            ...
                            {
                                blocks.Add(PrepareTextLine(env.Fonts.TooltipContentFont, "动作: " + kv.Key, ref current, Color.White, ref size.X));
                            }
                        }
                        size.Y = current.Y;
                    }
                ...
                case RenderObjectType.Portal:
                    {
                        ...
                        Vector2 current = Vector2.Zero;
                        blocks.Add(PrepareTextLine(env.Fonts.TooltipContentFont, "名称: " + p.PortalName, ref current, Color.White, ref size.X));
                        string pTypeName = GetPortalTypeString(p.PortalType);
                        blocks.Add(PrepareTextLine(env.Fonts.TooltipContentFont, "类型: " + p.PortalType + (pTypeName == null ? null : (" (" + pTypeName + ")")), ref current, Color.White, ref size.X));
                        stringLinker.StringMap.TryGetValue(p.ToMap, out sr);
                        blocks.Add(PrepareTextLine(env.Fonts.TooltipContentFont, "移动地图: " + (sr == null ? "(null)" : sr.Name) + "(" + p.ToMap + ")", ref current, Color.White, ref size.X));
                        blocks.Add(PrepareTextLine(env.Fonts.TooltipContentFont, "移动传送门: " + p.ToName, ref current, Color.White, ref size.X));
                        if (!string.IsNullOrEmpty(p.Script))
                            blocks.Add(PrepareTextLine(env.Fonts.TooltipContentFont, "文本: " + p.Script, ref current, Color.White, ref size.X));
                        size.Y = current.Y;
                    }
                    break;
            }
